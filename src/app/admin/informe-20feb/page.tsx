...
export default function Informe20FebPage() {
  ...
  const fetchReport = useCallback(async (): Promise<boolean> => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/admin/reports/tickets-20feb', { credentials: 'include' });
      if (res.status === 401) {
    ...
    <main className="min-h-screen bg-white text-gray-900">
      <div className="mx-auto max-w-3xl px-6 py-10 print:py-6">
        ...
        <table className="w-full border-collapse border border-gray-300 text-left">
          <thead>
            <tr className="bg-gray-100">
              ...
              <th className="border border-gray-300 px-4 py-3 font-semibold text-right">Regalados</th>
          ...
          <tbody>
            {data.by_type.map((row) => (
              <tr key={row.tipo} className="border-b border-gray-200">
                ...
                <td className="border border-gray-300 px-4 py-3 text-right">{row.regalados}</td>
            ...
            <tr className="bg-gray-50 font-semibold">
              ...
              <td className="border border-gray-300 px-4 py-3 text-right">{data.total_regalados}</td>
              <td className="border border-gray-300 px-4 py-3 text-right">
        ...
        <p className="mt-8 text-xs text-gray-500 print:mt-6">
          Documento generado el {formatReportDateTime(data.generated_at)}. Solo uso interno.
        </p>

        <p className="mt-6 text-sm text-gray-500 print:hidden">
          <Link href="/admin/stock" className="underline">Volver al stock</Link>
          {' · '}
          <button type="button" onClick={() => fetchReport()} className="underline">
            Actualizar informe
          </button>
        </p>
      </div>
